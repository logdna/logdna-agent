...
const checkHealth = (config) => {
    if (config.req) {
        return async.retry({
            ...
            , errorFilter: (error) => {
                return config.DEFAULT_HTTP_ERRORS && config.DEFAULT_HTTP_ERRORS.includes(error.code);
            }
        }, (reqCallback) => {
            return config.req({
                ...
                return reqCallback(response && response.statusCode >= config.ERROR_STATUS_CODES.INTERVAL_SERVER_ERROR ? {
                    code: config.DEFAULT_HTTP_ERRORS.slice(-1)
                } : error, Object.assign({}, JSON.parse(body || '{}'), error ? {} : {
                    statusCode: response.statusCode
                }));
            });
        }, (error, body) => {
            if (error) {
                utils.log(`Error from HealthCheck Endpoint: ${error}`);
            } else if (body && body.statusCode === 200) {
                if (body.autoupdate === true) {
                    if (config.autoupdate !== 0) {
                        utils.log('LogDNA Agent Update Request Received - Auto Updating...');
                        return utils.updateSelf();
                    }
                } else if (body.restart === true) {
                    utils.log('LogDNA Agent Restart Request Received - Restarting...');
