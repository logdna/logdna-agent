...
// TODO(darinspivey) This should have a callback so that we know when it's done.  Currently, we have to `sleep`
  ...
  if (config.req) {
    ...
    , errorFilter: (error) => {
      ...
      , qs: {
        ...
        return reqCallback(null, {...JSON.parse(body || '{}'), statusCode: response.statusCode})
      })
    }, (error, body) => {
      if (error) {
      ...
      config.healthcheckTimer = setTimeout(() => {
...
function connectLogServer(config) {
  firstrun = true
  const logurl = utils.buildUrl(config.LOGDNA_LOGHOST, config.LOGDNA_LOGPORT, config.LOGDNA_LOGSSL)
  ...
  linebuffer.setConfig(config)
  config.flushTimer = setTimeout(linebuffer.flush, config.FLUSH_INTERVAL)
  return checkHealth(config)
}
