...
module.exports.connectLogServer = (config, programName, callback) => {
    logurl = (config.LOGDNA_LOGSSL ? 'https://' : 'http://') + config.LOGDNA_LOGHOST + ':' + config.LOGDNA_LOGPORT;
    firstrun = true;
    ...
    debug(logurl);
    req = linebuffer.setConnection(logurl, options);
    return callback(null, checkHealth(config));
};
