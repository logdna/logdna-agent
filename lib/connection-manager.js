    ...
    if (config.req) {
        return async.retry({
            ...
            if (error) {
                ...
                if (body.endpoint) {
                    config.LOGDNA_LOGURL = config.LOGDNA_LOGURL.replace(config.LOGDNA_LOGHOST, body.endpoint);
                }
            } else {
...
const connectLogServer = (config) => {
    firstrun = true;
    config.LOGDNA_LOGURL = `${config.LOGDNA_LOGSSL ? 'https' : 'http'}://${config.LOGDNA_LOGHOST}:${config.LOGDNA_LOGPORT}`;
    debug(`LogDNA URL: ${config.LOGDNA_LOGURL}`);
