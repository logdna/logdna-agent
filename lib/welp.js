...
CustomWELR.prototype.streamEvents = () => {
    ...
    this.options.events.forEach((event) => {
        var logs = '';
        var PS = 'powershell -ExecutionPolicy Bypass "' + _this.script + ' ' + event + '"';
        this.powershell = exec(PS);
        this.powershell.stdout.on('data', (data) => {
            logs += data;
            const checkedLogs = _this._checkLogString(logs, event);
            if (checkedLogs) {
                _this.subscribers.data.forEach((subscriber) => {
                    subscriber.call(_this, checkedLogs);
                });
                logs = '';
            }
        });
        this.powershell.stderr.on('error', (error) => {
            _this.subscribers.error.forEach((subscriber) => {
                subscriber.call(_this, error);
            });
        });
    });
};
