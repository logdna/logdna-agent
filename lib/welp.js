...
CustomWELR.prototype._processWinEventData = function(logData, logName) {
    try {
        const parsedJSON = JSON.parse(logData);
        return _.isArray(parsedJSON) ? _.map(parsedJSON, (log) => {
            return JSON.stringify(log);
        }) : [logName + ': ' + JSON.stringify(parsedJSON)];
    } catch (e) {
        if (logData[0] === '[') {
            return _.map(_.trim(_.trim(logData, '\n]'), '[\n').split('},\n    {'), (log) => {
                return logName + ': {' + _.trim(_.trim(log, '}'), '{') + '}';
            });
        } else if (logData[0] === '{') {
            return [logName + ': {' + _.trim(_.trim(logData, '}'), '{') + '}'];
        } else {
            return [];
        }
    };
...
//        logData += data;
        if (!_.isEmpty(data)) {
            const logs = this._processWinEventData(data, options.event);
