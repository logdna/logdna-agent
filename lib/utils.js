...
const log = (message, level) => {
    ...
    Number.prototype.padStart = function(expectedLength) {
        const strNumber = String(Math.abs(this)).padStart(expectedLength, '0');
        return (this < 0) ? `-${strNumber}` : strNumber;
    };
    const date = `${dateObject.getFullYear()}-${(dateObject.getMonth() + 1).padStart(2)}-${dateObject.getDate().padStart(2)}`;
    const time = `${dateObject.getHours().padStart(2)}:${dateObject.getMinutes().padStart(2)}:${dateObject.getSeconds().padStart(2)}`;
    const timezone = (dateObject.getTimezoneOffset() * -5 / 3).padStart(4);
