...
function merge(objects) {
  return objects.reduce((merged, obj) => {
    ...
    merged.push(arr.filter((element, index) => { return arr.indexOf(element) === index }))
    return merged.filter((element, index) => { return merged.indexOf(element) === index })
  }, [[]])[0]
}
...
function padStart(number, expectedLength) {
...
function log(message, level) {
  ...
  const date = `${dateObject.getFullYear()}-${
    padStart(dateObject.getMonth() + 1, 2)
  }-${padStart(dateObject.getDate(), 2)}`
  const time = `${padStart(dateObject.getHours(), 2)}:${
    padStart(dateObject.getMinutes(), 2)
  }:${padStart(dateObject.getSeconds(), 2)}`
...
function pick2list(options, config) {
  const lowOptions = options.map((value) => { return value.toLowerCase() })
...
function processOption(options, config, hasWhitespace) {
  const newValues = merge(options.map((option) => {
    return split(option, ',', hasWhitespace)
  }))
  ...
  const diff = newValues.filter((value) => {
    return oldValues.indexOf(value) < 0
  }).filter(Boolean)

  return {
    values: merge([oldValues, newValues]).filter((element) => { return element })
...
function unsetConfig(options, config) {
  options = merge(options.map((option) => {
    return split(option, ',', false).filter((element) => { return element !== 'key' })
  }))
  const lowOptions = options.map((value) => { return value.toLowerCase() })
  if (lowOptions.indexOf('all') > -1) {
    return {
  ...
  const diff = oldValues.filter((value) => {
    return newValues.indexOf(value) < 0
  }).filter(Boolean)
