...
exports.compressWinLogObject = function(logObject) {
...
exports.complexSplit = function(str, delimiter) {
...
exports.merge = function(objects, is_json) {
...
exports.processWinEventOption = function(programValues, configValues) {
    ...
    configValues = (configValues ? (_.isString(configValues) ? exports.split(configValues) : configValues) : []);

    var cSOldValues = _.map(configValues, (config) => {
        return _.mapValues(exports.complexSplit(config, ' & '), (value) => {
            return exports.split(value[0], ';');
        });
    });

    var included = false;

    cSOldValues = _.map(cSOldValues, (oldValuesJSON) => {
        ...
        if (_.isEqual(newProviderNames, oldProviderNames) && !_.isEmpty(newLogNames) && !_.isEmpty(oldLogNames)) {
            oldValuesJSON.l = _.uniq(oldLogNames.concat(newLogNames));
            included = true;
        } else if (_.isEqual(newLogNames, oldLogNames) && !_.isEmpty(newProviderNames) && !_.isEmpty(oldProviderNames)) {
            oldValuesJSON.p = _.uniq(oldProviderNames.concat(newProviderNames));
            included = true;
        }
