...
module.exports.getAuthToken = function(config, agentName, socket) {
    ...
    if (proxy) {
        ...
        if (url.startsWith('https') && proxy.startsWith('https')) {
            agent = tunnel.httpsOverHttps({
                proxy: {
                    host: proxy
                }
            });  
        } else if (url.startsWith('https') && proxy.startsWith('http')) {
            agent = tunnel.httpsOverHttp({
                proxy: {
                    host: proxy
                }
            });  
        } else if (url.startsWith('http') && proxy.startsWith('https')) {
            agent = tunnel.httpOverHttps({
                proxy: {
                    host: proxy
                }
            });  
        } else if (url.startsWith('http') && proxy.startsWith('http')) {
    ...
    return got.post(url, { body: postdata, json: true, headers: { 'user-agent': config.UA}, agent: agent})
    .then(res => {
        var body = res.body;
        debug(body);
