...
function parseLine(line) {
    ...
    try {
        ...
        //console.log(`Got time ${t} from ${lineObj.time}`);
        if (lineObj.msg) {
            lineObj.message = lineObj.msg;
        }
        //console.log(`New message is ${lineObj.message}`);
        if (lineObj.level) {
        ...
        //console.log(`New level is ${lineObj.level}`);
    ...
    logfiles.forEach((file) => {
        ...
        if (os.platform() !== 'win32' && config.TAIL_MODE === 'unix') {
            ...
            tail.pipe(new Splitter())
                .on('data', (line) => {
                    req = parseLine(line);
                    //console.log(`Got req: ${JSON.stringify(req)}`);
                    msg = {t: req.t, l: req.l, f: file, label: labels};
                    //console.log(`Adding msg to buffer: ${JSON.stringify(msg)}`);
