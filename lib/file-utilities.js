...
const getFiles = (config, dir, callback) => {
    ...
    let globpattern = `{${globdir}/**/*.log,${globdir}/**/!(*.*)}`;
...
const streamFiles = (config, logfiles, callback) => {
    logfiles.forEach((file) => {
        ...
        if (os.platform() !== 'win32' && config.TAIL_MODE === 'unix') {
            debug('tailing: ' + file);
            const tail = spawn('tail', ['-n0', '-F', file]);
            tail.stdout.on('data', (data) => {
                data = data.toString().trim().split('\n');
                for (let line of data) {
                    line = { t: Date.now(), l: line, f: file, label: labels };
                    linebuffer.addMessage(line);
                }
            });
            ...
            debug(`tailing: ${file}`);
            const tail = TailReadStream.tail(file, config);
...
const streamAllLogs = (config, callback) => {
    ...
    async.each(config.logdir, (dir, done) => {
        getFiles(config, dir, (err, logfiles) => {
            if (!err && logfiles.length > 0) {
                ...
                const diff = logfiles.filter(element => files.indexOf(element) < 0);
    ...
    if (config.usejournald && firstrun) {
        ...
        if (config.usejournald === 'files') {
            const journalctl = spawn('journalctl', ['-n0', '-D', '/var/log/journal', '-o', 'json', '-f']);
        } else {
            const journalctl = spawn('journalctl', ['-n0', '-o', 'json', '-f']);
        }
        ...
        journalctl.stdout.on('data', (data) => {
            ...
            for (const line of data) {
                if (data[i].substring(0, 1) === '{' && line.substring(line.length - 1) === '}') {
                    // full chunk
                    linebuffer.addMessage(processChunk(line));
                    if (lastchunk) { lastchunk = null; } // clear

                } else if (line.substring(0, 1) === '{') {
                    // starting chunk
                    lastchunk = (lastchunk ? lastchunk : '') + line;

                } else if (lastchunk && line.substring(line.length - 1) === '}') {
                    // ending chunk
                    lastchunk += line;
