...
const streamFiles = (conn, config, logfiles, callback) => {
        ...
        if (os.platform() !== 'win32' && config.TAIL_MODE === 'unix') {
            ...
            tail.stdout.on('data', (data) => {
                ...
                for (i = 0; i < data.length; i++) {
                    ...
                    linebuffer.addMessage(conn, data[i]);
                }
            });
...
const streamAllLogs = (conn, config, callback) => {
    var newfiles = [];
    ...
    async.each(config.logdir, (dir, done) => {
        ...
        streamFiles(conn, config, newfiles, () => {
            return callback && callback();
        });
    });
        ...
        journalctl.stdout.on('data', (data) => {
            ...
            for (i = 0; i < data.length; i++) {
                if (data[i].substring(0, 1) === '{' && data[i].substring(data[i].length - 1) === '}') {
                    // full chunk
                    linebuffer.addMessage(conn, processChunk(data[i]));
                    ...
                    // ending chunk
                    lastchunk += data[i];
                    linebuffer.addMessage(conn, processChunk(lastchunk));
