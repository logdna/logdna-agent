        ...
        journalctl.stdout.on('data', function(data) {
            ...
            for (i = 0; i < data.length; i++) {
                if (data[i].substring(0, 1) === '{') {
                    ...
                    if (data[i].__REALTIME_TIMESTAMP && parseInt(data[i].__REALTIME_TIMESTAMP) > 10000000000000) {
                        // convert to ms
                        data[i].__REALTIME_TIMESTAMP = parseInt(data[i].__REALTIME_TIMESTAMP / 1000);
                    }
                    data[i] = { t: data[i].__REALTIME_TIMESTAMP, l: data[i].MESSAGE, f: data[i].CONTAINER_NAME || data[i].SYSLOG_IDENTIFIER || 'UNKNOWN_SYSTEMD_APP', pid: data[i]._PID && parseInt(data[i]._PID), prival: data[i].PRIORITY && parseInt(data[i].PRIORITY), containerid: data[i].CONTAINER_ID_FULL };
                } else {
                    data[i] = { t: Date.now(), l: data[i], f: 'systemd' };
                }
                linebuffer.addMessage(data[i]);
            }
        });
