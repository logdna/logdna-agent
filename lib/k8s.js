...
// Variables
let client;
let containerIdToNetworkId = {};
let networkIdToLabels = {};
let numLabels = 0;
let startup = true;
let lastChunk, bytesLeft;

let storeLabels = function(id, labels) {
    // Pick up actual container id from pod sandbox and map to networkid
    if (labels[SANDBOX_NAME_KEY]) {
        let networkId = labels[SANDBOX_NAME_KEY];
        ...
        if (labels.image) {
            ...
            if (!nidLabels.image) {
                let imageArray = labels.image.split(':');
...
let processAllContainers = function(containers) {
...
let processContainerEvent = function(event) {
...
let processDataStream = function(data) {
    let index = 0;
    if (data.indexOf('HTTP/1.1 ') === 0) {
        let responseCode = data.slice(0, data.indexOf('\r\n')).toString().split(' ')[1];
        if (parseInt(responseCode) >= 400) {
            log(`unexpected response code connecting to ${config.SOCKET_PATH}: ${responseCode}`, 'warn');
        }
        index = data.indexOf('\r\n\r\n');
        if (index !== -1) {
    ...
    if (data.length > 5) {
        ...
        let payload = (lastChunk || '') + data.slice(0, length).toString().trim();
...
exports.init = function() {
    ...
    client.on('error', function(error) {
        log(`error connecting to ${config.SOCKET_PATH}: ${error}`, 'error');
    });
    client.on('data', processDataStream);
    client.on('end', function() {
        log(`disconnected from ${config.SOCKET_PATH}`, 'warn');
    });
};

exports.getLabelsFromFile = function(filename) {
    let containerId = filename.substring(filename.lastIndexOf('-') + 1, filename.length - 4);
