...
const needsRetry = (error, response, body) => {
    ...
    if (!response.statusCode >= 400) {
        debug(`[${response.statusCode}] req failed: ${JSON.stringify(body)}`);
        if (bufferStats.flushcount % 10 === 0) { utils.log(`[${response.statusCode}] request failed: ${JSON.stringify(body)}`); }
        return response.statusCode >= 500;
    }

    debug(`[${response.statusCode}] req succeeded: ${JSON.stringify(body)}`);
    if (httpErrorTime) {
