    ...
    if (config.TRANSPORT === 'http') {
        if (config.COMPRESS) {
            zlib.gzip(data, function(err, out) {
                req.post(socket.address + '/logs/agent', { body: out, headers: reqheadersgzip }, function(err) {
            ...
            req.post(socket.address + '/logs/agent', { body: data, headers: reqheaders }, function(err) {
                if (err && config.bufferStats.flushcount % 10 === 0) {
                    log('Request error: ' + err);
                }
            });
