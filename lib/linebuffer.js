    ...
    if (config.TRANSPORT === 'http') {
        ...
        if (config.COMPRESS) {
            zlib.gzip(data, { level: config.GZIP_COMPRESS_LEVEL }, function(err, out) {
                req.post(socket.address + config.LOGDNA_LOGPATH, { body: out, headers: reqheadersgzip, qs: { timestamp: Date.now() }}, posthandler);
                out = null;
            });

        } else {
            req.post(socket.address + config.LOGDNA_LOGPATH, { body: data, headers: reqheaders, qs: { timestamp: Date.now() }}, posthandler);
        }
