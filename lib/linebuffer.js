    ...
    if (config.TRANSPORT === 'http') {
        ...
        if (config.COMPRESS) {
            zlib.gzip(data, { level: config.GZIP_COMPRESS_LEVEL }, function(err, out) {
                req.post(socket.address + config.LOGDNA_LOGENDPOINT, { body: out, headers: reqheadersgzip, qs: { timestamp: Date.now() }}, posthandler);
                out = null;
            });

        } else {
            req.post(socket.address + config.LOGDNA_LOGENDPOINT, { body: data, headers: reqheaders, qs: { timestamp: Date.now() }}, posthandler);
        }
...
module.exports.setConnection = (logurl, options) => {
    if (!req) {
        req = request.defaults({
            agent: (options.agent ? options.agent : new Agent({
