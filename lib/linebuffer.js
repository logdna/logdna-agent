...
const checkResponse = (error, response, body) => {
    if (error) {
        ...
        if (bufferStats.flushcount % 10 === 0) { utils.log('request error: ' + err); }
        return config.HTTP_RETRY && config.DEFAULT_HTTP_ERRORS && config.DEFAULT_HTTP_ERRORS.includes(err.code);
    }
