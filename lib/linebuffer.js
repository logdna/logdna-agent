...
const needsRetry = (error, response, body) => {
    if (error) {
        ...
        if (!httperrtime) { httperrtime = Date.now(); }
...
const flush = () => {
    ...
    if (httperrtime && config.TRANSPORT === 'http') {
        if (Date.now() - httperrtime < config.HTTP_TIMEOUT / 2) {
            dccount = buf.length;
            return;
        } else {
            httperrtime = Date.now(); // reset
