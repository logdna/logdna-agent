...
const needsRetry = (error, response, body) => {
    if (error) {
        ...
        if (!http_error_time) { http_error_time = Date.now(); }
...
const flush = () => {
    ...
    if (http_error_time && config.TRANSPORT === 'http') {
        if (Date.now() - http_error_time < config.HTTP_TIMEOUT / 2) {
            dccount = buf.length;
            return;
        } else {
            http_error_time = Date.now(); // reset
