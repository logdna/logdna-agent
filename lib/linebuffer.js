    ...
    if (config.TRANSPORT === 'http') {
        if (config.COMPRESS) {
            zlib.gzip(data, function(err, out) {
                req.post(socket.address + '/logs/agent', { body: out, headers: reqheadersgzip, auth: { user: 'x', pass: config.key }}, function(err) {
            ...
            req.post(socket.address + '/logs/agent', { body: data, headers: reqheaders, auth: { user: 'x', pass: config.key }}, function(err) {
                if (err && config.bufferStats.flushcount % 10 === 0) {
                    log('Request error: ' + err);
                }
            });
