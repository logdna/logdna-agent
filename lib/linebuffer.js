...
const needsRetry = (error, response, body) => {
    ...
    if (!response.statusCode >= config.ERROR_STATUS_CODES.CLIENT_ERROR) {
        debug(`[${response.statusCode}] req failed: ${bodyString}`);
        if (bufferStats.flushcount % 10 === 0) { utils.log(`[${response.statusCode}] request failed: ${bodyString}`); }
        return response.statusCode >= config.ERROR_STATUS_CODES.INTERVAL_SERVER_ERROR;
    }

    debug(`[${response.statusCode}] req succeeded: ${bodyString}`);
    if (httpErrorTime) {
