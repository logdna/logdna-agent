        ...
        it('streams new file rescan using TAIL_MODE trs', function() {
            ...
            return new Promise((resolve) => {
                ...
                fileUtilities.streamAllLogs(config, function() {
                    ...
                    fs.writeFileSync(path.join(tempDir, 'streamtest6.log'), 'arbitraryData3\n');
                });
            });
        });

        it('streams new file rescan using TAIL_MODE lib', function() {
            const MockWebSocket = require('mock-socket').WebSocket;
            const MockServer = require('mock-socket').Server;
            var server = new MockServer('ws://localhost:3011');
            var socket = new MockWebSocket('ws://localhost:3011');
            socket.connected = true;
            var config = {
                logdir: [tempDir],
                TRANSPORT: 'websocket',
            ...
            return new Promise((resolve) => {
                ...
                server.on('message', data => {
                    ...
                    if (count === 0) {
                        ...
                        assert(message.ls[0].l, 'arbitraryData4');
                    }
                    count += message.ls.length;
                ...
                fileUtilities.streamAllLogs(config, function() {
                    ...
                    setTimeout(function() {
                        fs.appendFileSync(path.join(tempDir, 'streamtest6.log'), 'arbitraryData4\n'); // only this line makes it due to rescan
